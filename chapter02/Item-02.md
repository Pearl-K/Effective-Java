...
- 빌더 패턴이 안전하다고 하는 이유로, 필수 매개변수를 무조건 받아야하는 것 외에, 유효성 검사 측면에서도 안전함이 추가된다.
- 빌더에 유효성 검사 코드를 추가하면 잘못된 매개변수를 최대한 일찍 발견하여 객체 생성 시점부터 막을 수 있다.
...
public abstract class Pizza {
    ...
    abstract static class Builder<T extends Builder<T>> {
        ...
        // self() + 재귀적 제네릭
        // 하위 타입 스스로를 반환하게 하여 타입 안정성을 유지한다.
...
| **필드 수가 많지만 유효성은 Spring Validation 만만으로 해결할 때** | ex. `@NotNull`, `@Min`, `@Size` 등으로 유효성 검사 위임 |
...
* 이는 DTO를 위한 것이며, **JPA 엔티티의 생성 유효성 보장에는 약하다**
